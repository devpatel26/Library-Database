...
export default function ItemEntry() {
  ...
  return (
    <div className="flex flex-col gap-8 lg:flex-row">
      {/* Main Content Area */}
      <main className="flex-1 space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:sticky lg:top-8 lg:w-72 lg:self-start">
        <div className="mb-2">
          <h1 className="text-4xl font-bold tracking-tight text-slate-900">
            Item Management
          </h1>
          <nav className="space-y-1 flex justify-evenly pt-4">
            {navLinks.map((link) => (
              <NavLink
                key={link.to}
                to={link.to}
                className={({ isActive }) =>
                  `block rounded-xl px-4 py-2.5 text-md font-medium transition-all ${
                    isActive
