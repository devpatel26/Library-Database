...
export default function ForgotPassword() {
  ...
  return (
    <section className="mx-auto flex w-full max-w-xl flex-col items-center rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm sm:p-12">
      ...
      {token ? (
        <form className="flex w-full flex-col items-center" onSubmit={HandleResetPassword}>
        ...
        <form className="flex w-full flex-col items-center" onSubmit={HandleRequestReset}>
          <label htmlFor="recovery-email" className={labelClasses}>Email Address</label>
          <input
            required
            id="recovery-email"
            name="email"
      ...
      {!token && previewLink && (
        <div className="mt-8 w-full max-w-md rounded-2xl border border-sky-200 bg-sky-50 p-6 text-left">
          ...
          <a href={previewLink} className="mt-2 block break-all text-sm font-medium text-sky-600 underline hover:text-sky-800">
            {previewLink}
          </a>
          {previewExpiresAt && (
